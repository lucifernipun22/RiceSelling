...
class CategoryRepository(var baseActivity: BaseActivity) {

    private var couponRepository = MutableLiveData<CategoryListModel>()

    fun hitCategoryApi(
        context: Context
    ): MutableLiveData<CategoryListModel> {
        if (Utiles.Utiles.internetChack()) {
            val call: Call<CategoryListModel> =
                    ...
                    if (response.isSuccessful) {
                        if (response.code() == Constants.STATUS_OK) {
                            if (response.body() != null) {
                                val body: CategoryListModel? = response.body()
