...
class GetMessageRepository(var baseActivity: BaseActivity) {

    private var messageResponse = MutableLiveData<GetMessagesModel>()

    fun hitGetMessageApi(
        token: String,
        context: Context
    ): MutableLiveData<GetMessagesModel> {
        if (Utiles.Utiles.internetChack()) {
            val call: Call<GetMessagesModel> =
                baseActivity.apiService!!.getMessage("Bearer ${token}")
            call.enqueue(object : Callback<GetMessagesModel?> {
                    ...
                    if (response.isSuccessful) {
                        if (response.code() == Constants.STATUS_OK) {
                            if (response.body() != null) {
                                val body: GetMessagesModel? = response.body()
