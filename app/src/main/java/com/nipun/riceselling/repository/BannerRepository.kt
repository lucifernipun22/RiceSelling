...
class BannerRepository(var baseActivity: BaseActivity) {

    private var couponRepository = MutableLiveData<BannerModel>()

    fun hitBannerApi(
        context: Context
    ): MutableLiveData<BannerModel> {
        if (Utiles.Utiles.internetChack()) {
            val call: Call<BannerModel> =
                baseActivity.apiService!!.getBanner()
                    ...
                    if (response.isSuccessful) {
                        if (response.code() == Constants.STATUS_OK) {
                            if (response.body() != null) {
                                val body: BannerModel? = response.body()
