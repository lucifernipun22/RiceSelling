...
class ProductDetailRepository(var baseActivity: BaseActivity) {

    private var couponRepository = MutableLiveData<ProductDetailModel>()

    fun hitProductDetailApi(
        id: String,
        context: Context
    ): MutableLiveData<ProductDetailModel> {
        if (Utiles.Utiles.internetChack()) {
            val call: Call<ProductDetailModel> =
                baseActivity.apiService!!.getProductDetail(id)
                    ...
                    if (response.isSuccessful) {
                        if (response.code() == Constants.STATUS_OK) {
                            if (response.body() != null) {
                                val body: ProductDetailModel? = response.body()
