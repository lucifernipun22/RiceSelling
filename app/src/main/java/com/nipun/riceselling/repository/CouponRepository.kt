...
class CouponRepository(var baseActivity: BaseActivity) {

    private var couponRepository = MutableLiveData<CouponModel>()

    fun hitCouponApi(
        token: String,
        context: Context
    ): MutableLiveData<CouponModel> {
        if (Utiles.Utiles.internetChack()) {
            val call: Call<CouponModel> =
                baseActivity.apiService!!.getCouponList("Bearer ${token}")
            baseActivity.startProgress()
            call.enqueue(object : Callback<CouponModel?> {
                @SuppressLint("NullSafeMutableLiveData")
                override fun onResponse(
                    call: Call<CouponModel?>,
                    response: Response<CouponModel?>
                ) {
                    baseActivity.stopProgress()
                    if (response.isSuccessful) {
                        if (response.code() == Constants.STATUS_OK) {
                            if (response.body() != null) {
                                val body: CouponModel? = response.body()
