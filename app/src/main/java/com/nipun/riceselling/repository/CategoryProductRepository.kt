...
class CategoryProductRepository(var baseActivity: BaseActivity) {

    private var couponRepository = MutableLiveData<CategoryProductModel>()

    fun hitCategoryProductApi(
        id: String,
        context: Context
    ): MutableLiveData<CategoryProductModel> {
        if (Utiles.Utiles.internetChack()) {
            val call: Call<CategoryProductModel> =
                baseActivity.apiService!!.getCategoryProductList(id)
                    ...
                    if (response.isSuccessful) {
                        if (response.code() == Constants.STATUS_OK) {
                            if (response.body() != null) {
                                val body: CategoryProductModel? = response.body()
